...
function MetricCard({ icon, label, value }) {
  return (
    <div className="card p-4 flex min-w-0 items-start gap-3">
      <div className="shrink-0 w-9 h-9 rounded-lg bg-brand/10 flex items-center justify-center text-brand">
        {icon}
      </div>
      <div className="min-w-0">
        <p className="text-xs text-gray-500 uppercase tracking-wider mb-0.5 truncate">{label}</p>
  ...
  return (
    <div className="min-w-0 space-y-5 pb-10 sm:space-y-6">
        ...
        <div className="flex min-w-0 flex-col items-center gap-6 md:flex-row md:gap-8">
          ...
          <div className="w-full min-w-0 flex-1 space-y-2">
            {[
              ...
              return (
                <div key={level} className="flex min-w-0 items-center gap-2 sm:gap-3">
                  <span className="w-12 shrink-0 text-right text-xs text-gray-400 sm:w-14">{label}</span>
                  ...
                  <span className="w-4 shrink-0 text-xs font-mono text-gray-400">{count}</span>
                </div>
              );
            })}
          </div>
      ...
      <div className="card min-w-0 p-4 sm:p-6">
        ...
        <p className="break-words text-sm leading-relaxed text-gray-300">{summary}</p>
      </div>
